        ...
        private void buttonLlamar_Click(object sender, EventArgs e)
        {
            ...
            if (this.flagNumeral)
            {
                try
                {
                    ...
                    this.centralita += llamadaP;
                }
                catch(CentralitaException exc)
                {
                    MessageBox.Show(exc.Message + exc.NombreClase + exc.NombreMetodo, "Llamada existente", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            ...
            else
            {
                try
                {
                    ...
                    //Harcodeo para forzar el tryCatch:
                    //Local llamadaL = new Local("Lanús", 45, "San Rafael", 1.99f);
                    this.centralita += llamadaL;
                }
                catch(CentralitaException exc)
                {
                    MessageBox.Show(exc.Message + exc.NombreClase + exc.NombreMetodo, "Llamada existente", MessageBoxButtons.OK, MessageBoxIcon.Error);
